...
The Go CLI is a local OpenClaw exec secrets provider. It must authenticate to the official 1Password Go SDK with a service account token while avoiding raw secrets in argv, shell history, repository files, logs, or test fixtures.

Toolsmith guidance favors stable logical secret inputs, `ENV` and `*_FILE` delivery, fail-fast behavior when both forms are set, and provider-specific resolution at the edge. This repository tightens that model further for human local setup: raw service-account token import is interactive only, while runtime resolution and diagnostics must use the system keyring through `github.com/99designs/keyring`.
...
- Treat the service account token value env vars `OCOP_SERVICE_ACCOUNT_TOKEN` and `OCOP_SERVICE_ACCOUNT_TOKEN_FILE` as removed inputs that must fail fast when present.
- Make `token` prompt interactively with terminal echo disabled.
- Make `token` dry run by default and require `--prompt-and-save` for keyring mutation.
- Fail `token --prompt-and-save` when the keyring item already exists unless `--force` is provided.
- Fail closed in OpenClaw resolver mode when token loading fails or removed token env vars are present.
...
- Do not accept raw service account tokens through env vars or token files.
...
- Operators can import a token into the system keyring without putting it in shell command history, shell env, or token files.
