...
openclaw-1p-sdk-resolver openclaw audit [scan|suggest] [--path <openclaw.json>] [--repo <dir>] [--json]
...
- `openclaw audit scan` is read-only and reports:
  - existing `op://` usage
  - likely candidates that should move to 1Password
  - risky inline token patterns
  - fingerprints only (`len + sha256 prefix`), never raw secret values
- `openclaw audit suggest` provides safe provider/config migration guidance.

OpenClaw config path resolution precedence used by `openclaw audit`:
...
- `src/openclaw.ts`
  ...
  - Provides safe OpenClaw audit helpers (reference collection, repo candidate scan, suggestions).
  - Uses fingerprints only for secret-like findings (never raw values).
- `src/resolver.ts`
  - Entrypoint wiring:
